...
* Called from : PMENU.C
* Function    : Operate character's occurence on paper in textmode.
* Create date : nov.31
...
*   Some attributes are tokkle to itself. Some, effect to the others             
*   Example ,superscript attr, sets itself and also resets subscript attribute .  */
int tadj_attr( unsigned int *attr, int *ch );

/** Put ESC code according to attribute in 12 text buffers. */
void putattr( unsigned int *attr, int *current );

/** Print ESC code according to attribute */
void printattr( unsigned char attr );

/** Checking type of character appearance e.g. block, boldface, enlarge, normal ect. */
void tpretobuffer( unsigned int *attr, int *ch, int *current );

/** Invention of graphic blocks to build tables this include both single and double lineblock
*   such as ˜ ‘ ™ “ – ’ š  › • 
